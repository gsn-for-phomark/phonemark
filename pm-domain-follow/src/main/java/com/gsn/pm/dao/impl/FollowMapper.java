...
public interface FollowMapper extends MisBaseMapper<Followinfo> {
     ...
     */
    @Insert("insert into followinfo values(null,mno= #{mno},bno= #{bno},0,1)")
 ...
 * @return
 */
    @Select("<script>"+"select  a1.mno as fansno ,a1.nickName,a1.mpic,a1.msign,IFNULL(a1.messaynums,0) "+
            ...
            " (select mno from followinfo where 1=1  "+
            "<if test='fansno!=null'>"
            + "and bno = #{bno}"
            + "</if>"+
            " union all "+
            " select bno as mno from followinfo where 1=1  "+
            "<if test='fansno!=null'>"
            + "and mno = #{mno}"
            + "</if>"+
            " and status=2) f2 group by mno  ) a2 on a1.mno=a2.mno order by fansno asc "+
            "</script>")
    List<FollowList> findBeFollowedList(@Param("mno")Integer mno,@Param("bno") Integer bno);
    /**
     * 关注用户列表
     * @param mno,bno
     * @return
     */
    @Select("<script>"+"select  a1.mno as follno,a1.nickName,a1.mpic,a1.msign,IFNULL(a1.messaynums,0) "+
            " messaynums,IFNULL(a1.fnum,0) fnum,IFNULL(a1.bnum,0) bnum from  "+
            " (select m.mno,nickName,mpic,msign,messaynums,f.fnum,b.bnum from memberinfo  m left join (  "+
            " select mno,sum(fnum) as fnum from( "+
            " select mno,COUNT(bno) fnum from followinfo group by mno union all  "+
            ...
            " (select bno from followinfo where 1=1 "+
            "<if test='follno!=null'>"
            + "and mno = #{mno}"
            + "</if>"+
            " union all "+
            "select mno as bno from followinfo where 1=1 "+
            "<if test='follno!=null'>"
            + "and bno = #{bno}"
            + "</if>"+
